...
use crate::{collect::LatencyTrace, latency_trace_old::LatencyTraceOld};
use std::{future::Future, hint::black_box};

/// Set-up for measurement of latencies.
pub fn measure_latencies1(lt: LatencyTraceOld) {
...
pub fn measure_latencies2(lt: LatencyTraceOld, f: impl FnOnce()) -> usize {
    let ltp = LatencyTrace::activated(lt.0).unwrap();
    f();
...
pub fn measure_latencies2_tokio<F>(lt: LatencyTraceOld, f: impl FnOnce() -> F) -> usize
where
