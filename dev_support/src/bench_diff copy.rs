...
/// Compares the difference of total latency for two closures `f1(ctx)` and `f2(ctx)` in ***microseconds***.
...
///
/// Arguments:
/// - `ctx` - context used to set-up `f1` and `f2`.
/// - `f1` - closure that produces the first target for comparison, using `ctx` as input.
/// - `f2` - closure that produces the second target for comparison, using `ctx` as input.
